...
export type RatingBreakdownItem = { key: string; label: string; weight: number; points: number; completeness: Completeness };
...
type Rule = { key: string; label: string; weight: number; partial: number; evaluate: (task: TaskCard) => Completeness; recommendation: string };
...
const rules: Rule[] = [
  { key: "contextNeed", label: "Контекст и потребность", weight: 20, partial: 10, recommendation: "Добавьте контекст текущего процесса и чётко сформулируйте проблему.", evaluate: (t) => {
  ...
  { key: "data", label: "Данные и материалы", weight: 20, partial: 10, recommendation: "Перечислите доступные данные, их формат и примерный объём.", evaluate: (t) => {
    const value = text(t.dataMaterials); if (!meaningful(value)) return "EMPTY"; return value.length >= 35 && specific(value) ? "COMPLETE" : "PARTIAL";
  } },
  { key: "result", label: "Ожидаемый результат", weight: 15, partial: 8, recommendation: "Опишите конкретный результат первой демонстрации или итоговый артефакт.", evaluate: (t) => {
  ...
  { key: "success", label: "Критерии успеха", weight: 15, partial: 7, recommendation: "Добавьте измеримый или однозначно проверяемый критерий успеха.", evaluate: (t) => {
  ...
  { key: "constraints", label: "Ограничения", weight: 10, partial: 5, recommendation: "Укажите сроки, правила безопасности или технические ограничения.", evaluate: (t) => {
  ...
  { key: "users", label: "Пользователи", weight: 10, partial: 5, recommendation: "Назовите основную группу пользователей и её роль в процессе.", evaluate: (t) => {
  ...
  { key: "contact", label: "Связь с бизнесом", weight: 10, partial: 5, recommendation: "Укажите контактную роль и формат регулярной обратной связи.", evaluate: (t) => {
...
export function calculateTaskRating(task: TaskCard): TaskRating {
  const breakdown = rules.map((rule) => { const completeness = rule.evaluate(task); return { key: rule.key, label: rule.label, weight: rule.weight, completeness, points: completeness === "COMPLETE" ? rule.weight : completeness === "PARTIAL" ? rule.partial : 0 }; });
