...
function ScorePanel({ card, previous }: { card: TaskCard; previous?: number | null }) {
  ...
  return <aside className="score-panel">
    ...
    {result.missing.length > 0 && <div className="score-help"><b>Не заполнено полностью</b>{result.missing.slice(0, 4).map((item) => <span key={item}>○ {item}</span>)}</div>}
...
export function NexaApp() {
  ...
  const [confirmed, setConfirmed] = useState(false); const [dirty, setDirty] = useState(true); const [previousScore, setPreviousScore] = useState<number | null>(null); const [aiSource, setAiSource] = useState<"openai" | "fallback" | null>(null);
  ...
  async function saveAndRecalculate() {
    ...
    try {
      ...
      setPreviousScore(previous); setCard(data.task); setDirty(false); setConfirmed(false); setMyTasks((current) => [data.task!, ...current.filter((task) => task.id !== data.task!.id)]);
