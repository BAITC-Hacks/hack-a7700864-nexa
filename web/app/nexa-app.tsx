...
import { emptyCard, type Proposal, type TaskCard, type Team } from "@/lib/domain";

type Role = "business" | "student";
type View = "create" | "market" | "detail";
...
function ScorePanel({ card, previous }: { card: TaskCard; previous?: number | null }) {
  ...
  return <aside className="score-panel">
    ...
    <div className="score-breakdown">{result.breakdown.map((item) => <div key={item.key}><span>{item.points === item.weight ? "✓" : "△"} {item.label}</span><b>{item.points}/{item.weight}</b></div>)}</div>
    <div className="score-help"><b>Как повысить рейтинг</b>{result.breakdown.filter((i) => i.points < i.weight).slice(0, 3).map((i) => <span key={i.key}>+ Дополните: {i.label.toLowerCase()}</span>)}</div>
  </aside>;
}
...
export function NexaApp() {
  ...
  const [questions, setQuestions] = useState<string[]>([]); const [answers, setAnswers] = useState(["", "", ""]);
  ...
  async function analyzeDraft() {
    setBusy(true);
    try { const res = await fetch("/api/analyze", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ description }) }); const data = await res.json() as { error?: string; questions: string[]; source: string }; if (!res.ok) throw new Error(data.error); setQuestions(data.questions); setStage("questions"); toast.success(data.source === "openai" ? "AI сформировал 3 вопроса" : "3 вопроса готовы — используется надёжный fallback"); }
  ...
  async function publishTask() {
    ...
    try { const res = await fetch("/api/tasks", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(card) }); const data = await res.json() as { error?: string; task: TaskCard }; if (!res.ok) throw new Error(data.error); setTasks((current) => [data.task, ...current]); setSelectedId(data.task.id!); toast.success("Задача опубликована в каталоге"); navigate("detail"); }
  ...
  async function setProposalStatus(id: string, status: "ACCEPTED" | "REJECTED") {
    try { const res = await fetch(`/api/proposals/${id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ status }) }); const data = await res.json() as { error?: string }; if (!res.ok) throw new Error(data.error); setProposals((current) => current.map((item) => item.id === id ? { ...item, status } : item)); toast.success(status === "ACCEPTED" ? "Команда выбрана" : "Предложение отклонено"); }
