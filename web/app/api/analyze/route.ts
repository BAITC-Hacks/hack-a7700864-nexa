...
export async function POST(request: Request) {
  try {
    ...
    const result = input.questions ? await aiService.generateTaskCard(input.description, input.questions, input.answers ?? ["", "", ""]) : await aiService.analyzeDraft(input.description);
